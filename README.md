...
基于实际项目经验重新构建的中日技术文档翻译与质量校验工作流Demo。

## 项目背景

在智能驾驶项目实习期间，团队需要频繁处理中日双语技术文档。由于日常项目任务较多，人工翻译通常只能利用零散时间完成，一份约 50 页的文件往往需要跨 3 个工作日交付。

为缩短处理周期、减少重复操作，我开发了技术文档翻译 Skill，并通过飞书自建应用和 Feishu MCP Server 接入团队协作入口。投入使用后，约 50 页技术文档的处理时间缩短至 30–60 分钟。
...
本仓库是根据原项目规则重新实现的公开演示，并非直接复制原代码。

- 原 Skill 使用 OpenAI 模型完成领域分析、动态术语提取和中日双向翻译，并覆盖 DOCX、PDF、PPTX与XLSX。
...
本仓库不包含源代码、文档、数据、项目名称、会议记录、接口配置、API Key、App Secret 或其他内部信息。Excel 样例来自日本国土交通省公开资料。作品 Demo 的在线初译模式仅面向公开样例，禁止上传公司或客户的非公开文件。
