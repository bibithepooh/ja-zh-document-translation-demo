...
双击 `start_demo.bat`，浏览器会打开 `http://127.0.0.1:4173/`。选择 XLSX 后会生成可下载的翻译版和纯中文版。翻译版遵循“原文在前，蓝色小字号译文另起一行附在后”的规则，并保留源表的合并单元格、填充、边框、行列尺寸与下拉框。日文输入时附中文译文；中文输入时附日文译文。静态介绍页可发布到 GitHub Pages；完整文件处理功能需要运行本地后端。
